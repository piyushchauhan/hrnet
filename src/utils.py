...
def conv_2d(inputs, channels, kernel_size=3, strides=1, batch_normalization=True, activation=None,
    ...
    output = tf.compat.v1.layers.conv2d(inputs=inputs, filters=channels, kernel_size=kernel_size, strides=strides,padding=padding, name=name + '_conv', kernel_initializer=kernel_initializer)
    name = name + '_conv'

    if batch_normalization:
        output = tf.compat.v1.layers.batch_normalization(output, axis=-1, momentum=0.9, name=name+'_bn', training=is_training)
        name = name + '_bn'
...
def up_sampling(input, channels, method='nearest_neighbor', rate=2, name='', activation=leaky_Relu, is_training=True):
    ...
    if method == 'nearest_neighbor':
        ...
        output = tf.image.resize(input, size=(x*rate, y*rate), name=name + '_upsampling')
        name += '_upsampling'
